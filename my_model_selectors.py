        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                ...
                if(best_score is None or i_score < best_score):
                    best_score, best_components = i_score, n_components
...
#                self.print_exception(e, self.this_word, n_components)
        if(best_score == None):
            best_components = 3
        return self.base_model(best_components)
...
class SelectorDIC(ModelSelector):
    ...
    def logL_models_values(self):
        models, values = {}, {}
        for nn in range(self.min_n_components, self.max_n_components+1):
            ...
            for word in self.words.keys():
                ...
                try:
                    ...
                    model = GaussianHMM(n_components=nn, n_iter=1000).fit(X_i, lengths_i)
    ...
    def select(self):
        ...
        for nn in range(self.min_n_components, self.max_n_components+1):
            ...
            if(len(self.sequences) < 3):
                try:
                    model = GaussianHMM(n_components=nn, n_iter=1000).fit(self.X, self.lengths)
                    logL = model.score(self.X, self.lengths)
                ...
                for cv_train_idx, cv_test_idx in split_method.split(self.sequences):
                    ...
                    try:
                        model = GaussianHMM(n_components=nn, n_iter=1000).fit(X_train, lengths_train)
                ...
                temp_score = np.average(scores)
                if(best_score is None or temp_score > best_score):
                    best_score, best_n_components = temp_score, n_components
        if(best_score == None):
            best_n_components = 3
        return self.base_model(best_n_components)